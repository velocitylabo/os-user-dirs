...
console.log("Deno compatibility tests\n");
...
for (const [name, fn] of Object.entries(namedExports)) {
    test(`getPath("${name}") === ${name}()`, () => {
        assert.equal(getPath(name as Parameters<typeof getPath>[0]), fn());
    });
}
...
test("projectDirs paths contain app name", () => {
    const dirs = projectDirs("deno-test-app");
    for (const [key, val] of Object.entries(dirs)) {
        if (val !== null) {
            assert.ok((val as string).includes("deno-test-app"), `${key} should contain app name`);
        }
    }
});
...
test("projectDirs vendor option works", () => {
    ...
    for (const [key, val] of Object.entries(dirs)) {
        if (val !== null) {
            assert.ok((val as string).includes("my-app"), `${key} should contain app name`);
        }
    }
});
...
test("projectUserDirs paths contain app name", () => {
    const dirs = projectUserDirs("deno-test-app");
    for (const [key, val] of Object.entries(dirs)) {
        assert.ok((val as string).includes("deno-test-app"), `${key} should contain app name`);
    }
});
test("projectUserDirs paths match getPath + app name", () => {
    const dirs = projectUserDirs("test-app");
    assert.equal(dirs.downloads, path.join(getPath("downloads" as Parameters<typeof getPath>[0]), "test-app"));
    assert.equal(dirs.desktop, path.join(getPath("desktop" as Parameters<typeof getPath>[0]), "test-app"));
});
...
test("getAllDirs non-null values are absolute paths", () => {
    ...
    for (const [key, val] of Object.entries(dirs)) {
        if (val !== null) {
            assert.ok(path.isAbsolute(val as string), `${key} should be absolute`);
        }
    }
});
